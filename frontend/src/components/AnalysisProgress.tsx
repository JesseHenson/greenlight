...
export default function AnalysisProgress({ challengeId, onComplete }: Props) {
  const [status, setStatus] = useState<AnalysisStatus | null>(null);

  useEffect(() => {
    const interval = setInterval(async () => {
      try {
        const res = await api.get(`/challenges/${challengeId}/analysis-status`);
        setStatus(res.data);
  ...
  return (
    <div className="bg-white border border-slate-200/60 rounded-lg shadow-sm p-6 text-center">
      ...
      <p className="text-sm text-slate-500 mt-2">{pct}%</p>
    </div>
  );
}
